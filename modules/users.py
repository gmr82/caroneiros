...
class User(DraftInterface):
    ...
    @staticmethod
    def print_from(users: set[Any]| list[Any] | dict[str, Any]) -> None:
...
class Regular(User):
    ...
    def add_carpool(self, *args) -> bool | None:
        ...
        carpool = Carpool(origin, destination, driver_username, status)
    ...
    def find_carpool(self, *args) -> bool | None:
        ...
        key = re.sub(r"[\W]", "", key).lower()  # tentativa de filtro para manter apenas letras e números
...
class Admin(User):
    ...
    def set_debug_menu(self) -> Menu:
        ...
        options = list()  # ¿mudar p/ tupla?
        options.append(("Ver todos os usuários", self.access_print_regular_users, None))
        options.append(("Remover todos os usuários", print, None))
        options.append(("Ver todas as caronas", print, None))
        options.append(("Remover todas as caronas", print, None))
        options.append(("Voltar", "Voltando…"))
        return Menu(title=title, options=options, invalid_selection_text=invalid_selection_text)
    ...
    @staticmethod
    def users_keys_by_type(typ: type) -> set[Any]:
        """ filtrar burramente os usuários pelo tipo """
        keys = set()
        for key, user in users.items():
            if type(user) is typ:
                keys.add(key)
    ...
    def access_print_regular_users(self, *args) -> bool | None:
        keys = Admin.users_keys_by_type(Regular)
        for key in keys:
